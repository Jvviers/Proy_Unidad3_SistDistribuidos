...
$file = Join-Path -Path $outPath -ChildPath ("{0}-mariadb.txt" -f $timestamp)
...
Section "Health inicial (MariaDB)" (Invoke-RestMethod -Method Get -Uri "http://localhost:8000/health" | ConvertTo-Json -Depth 5)
Section "Health app3" (Invoke-RestMethod -Method Get -Uri "http://localhost:8003/health" | ConvertTo-Json -Depth 5)

$stopOut = (docker compose stop mariadb-primary 2>&1 | Out-String)
Section "Parada de primario (mariadb-primary)" $stopOut

$ready = "Timeout"
for ($i=0; $i -lt 10; $i++) {
    docker compose exec mariadb-replica mysqladmin ping -uroot -proot 2>&1 | Out-Null
    if ($LASTEXITCODE -eq 0) { $ready = "mariadb-replica READY intento $i"; break }
...
$logs = (docker compose logs --tail=200 mariadb-watchdog mariadb-haproxy 2>&1 | Out-String)
Section "Logs watchdog/HAProxy (MariaDB)" $logs
