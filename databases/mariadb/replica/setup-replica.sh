...
mysql -h"$LOCAL_HOST" -P"$LOCAL_PORT" -uroot -p"$MYSQL_ROOT_PASSWORD" -e "STOP SLAVE; RESET SLAVE ALL; CHANGE MASTER TO MASTER_HOST='${PRIMARY_HOST}', MASTER_USER='${REPL_USER}', MASTER_PASSWORD='${REPL_PASS}', MASTER_PORT=${PRIMARY_PORT}, MASTER_LOG_FILE='${FILE}', MASTER_LOG_POS=${POS}; START SLAVE; SET GLOBAL read_only=1;" || exit 1
