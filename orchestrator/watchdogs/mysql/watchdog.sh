...
promote_replica(){
  ...
  mysql -h"${REPLICA_HOST}" -uroot -p"${ROOT_PASS}" -e "STOP REPLICA; RESET REPLICA ALL; SET GLOBAL super_read_only=0; SET GLOBAL read_only=0;" || {
    log "Error al promover replica"; return 1; }

  if [ -S "$HAPROXY_SOCKET" ]; then
    ADDR=$(resolve_addr "$REPLICA_HOST")
    [ -z "$ADDR" ] && ADDR="$REPLICA_HOST"
    echo "disable server mysql_cluster/primary" | socat stdio "$HAPROXY_SOCKET" || true
    echo "set server mysql_cluster/primary addr ${ADDR} port 3306" | socat stdio "$HAPROXY_SOCKET" || true
    echo "enable server mysql_cluster/primary" | socat stdio "$HAPROXY_SOCKET" || true
